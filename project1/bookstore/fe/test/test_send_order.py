...
class TestSendReceive:
    ...
    def test_send_ok(self):
        code = self.seller.send_order(self.store_id, self.order_id[0])
        assert code == 200
    ...
    def test_error_store_id(self):
        code = self.seller.send_order(self.store_id + "null", self.order_id[0])
        assert code != 200

    def test_error_order_id(self):
        code = self.seller.send_order(self.store_id, self.order_id[0] + "null")
        assert code != 200

    def test_error_seller_id(self):
        self.seller.seller_id = self.seller.seller_id + "null"
        code = self.seller.send_order(self.store_id, self.order_id[0])
        assert code != 200
    ...
    def test_error_buyer_id(self):
        code = self.seller.send_order(self.store_id, self.order_id[0])
        assert code == 200
        self.buyer.user_id = self.buyer.user_id + "null"
        code = self.buyer.receive_order(self.order_id[0])
        assert code != 200

    def test_send_not_pay(self):
        code = self.seller.send_order(self.store_id, self.order_id[1])
        assert code != 200

    def test_receive_not_send(self):
        code = self.buyer.receive_order(self.order_id[1])
        assert code != 200

    def test_repeat_send(self):
        code = self.seller.send_order(self.store_id, self.order_id[0])
        assert code == 200
        code = self.seller.send_order(self.store_id, self.order_id[0])
        assert code != 200
